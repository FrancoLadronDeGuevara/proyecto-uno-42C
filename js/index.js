let listaUsuariosPredeterminada = [ // Lista de usuarios predeterminada
    {
...
const usuarioLogueado = JSON.parse(localStorage.getItem("usuarioLogueado")) || false; //Si existe usuarioLogueado en localStorage, la guardamos en la variable usuarioLogueado de lo contrario la inicializamos false
const adminLogueado = JSON.parse(localStorage.getItem("adminLogueado")) || false; //Si existe adminLogueado en localStorage, la guardamos en la variable adminLogueado de lo contrario la inicializamos false
...
if (usuarios.length === 0) { //Si la lista de usuarios esta vacia la inicializamos con la lista predeterminada
    localStorage.setItem("listaUsuarios", JSON.stringify(listaUsuariosPredeterminada))
}

if(usuarioLogueado){ //Si el usuario esta logueado
...
if(adminLogueado){ //Si el admin esta logueado
    btnIniciarSesion.classList.add("d-none"); //Ocultamos el boton de iniciar sesion
    btnRegistrarse.classList.add("d-none"); //Ocultamos el boton de registrarse
